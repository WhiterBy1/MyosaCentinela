...
static const char *APDS_DBG_TAG = "apds9960_lib";
...
uint8_t apds9960_read_gesture(apds9960_handle_t sensor)
{
    ...
    /* FIX 4: tope duro de tiempo total. Si hay un objeto quieto (no una
     * mano pasando) cerca del sensor, cada vuelta puede seguir viendo
     * "algo" de diferencia U/D o L/R (ruido/asimetría óptica), lo que
     * reinicia el cronómetro de 300ms una y otra vez sin parar: la función
     * nunca regresaba y por eso se congelaba TODO el programa (pantalla,
     * MPU, BMP incluidos), porque este while(1) nunca soltaba el control.
     * Con este tope, pase lo que pase, a los ~1.5s la función SIEMPRE
     * regresa (con o sin gesto resuelto). */
    TickType_t loop_start_tick = xTaskGetTickCount();
    const TickType_t max_total_ticks = pdMS_TO_TICKS(1500);

    ESP_LOGI(APDS_DBG_TAG, "=== inicio lectura de gesto (GVALID=1) ===");
    ...
    while (1) {
        ...
        if (!apds9960_gesture_valid(sensor)) {
            ESP_LOGI(APDS_DBG_TAG, "=== fin (GVALID paso a 0, sin gesto) ===");
            return 0;
        }

        if (xTaskGetTickCount() - loop_start_tick > max_total_ticks) {
            ESP_LOGW(APDS_DBG_TAG, "=== fin (tope duro de %lums alcanzado, objeto quieto muy cerca?) ===",
                     (unsigned long)(max_total_ticks * portTICK_PERIOD_MS));
            apds9960_reset_counts(sensor);
            return 0;
        }
        ...
        /* FIX 1: GFLVL es la cantidad de DATASETS (4 bytes c/u: U,D,L,R), no
         * la cantidad de bytes. El original leía "toRead" bytes en vez de
         * "toRead*4", así que solo drenaba una fracción del FIFO por
         * vuelta y dejaba datos viejos atascados (podía hasta desbordar
         * el FIFO). Con GFIFOTH=4, GVALID normalmente ya arranca con
         * varios datasets en cola, así que este bug afectaba casi siempre. */
        if (toRead > 32) {
            toRead = 32; /* tope de hardware del FIFO de gesto */
        }
        i2c_bus_read_bytes(sens->i2c_dev, APDS9960_GFIFO_U, toRead * 4, buf);

        /* FIX 2: "t" arrancaba en 0. En la primera vuelta, si todavía no
         * había un diff grande, "xTaskGetTickCount() - t" comparaba contra
         * el tick count real del sistema (que ya es enorme a esta altura
         * del programa) en vez de contra "hace cuánto empezó este gesto",
         * así que el timeout de 300ms se disparaba de inmediato y la
         * función abortaba antes de darle tiempo a la mano de moverse. */
        if (!t_initialized) {
            t = xTaskGetTickCount();
            t_initialized = true;
        }

        ESP_LOGI(APDS_DBG_TAG, "GFLVL=%d  FIFO[0..3] U=%d D=%d L=%d R=%d",
                 toRead, buf[0], buf[1], buf[2], buf[3]);
        ...
        ESP_LOGI(APDS_DBG_TAG, "ud_diff=%d lr_diff=%d | up_cnt=%d down_cnt=%d left_cnt=%d right_cnt=%d | gesture_parcial=%d",
                 up_down_diff, left_right_diff,
                 sens->up_cnt, sens->down_cnt, sens->left_cnt, sens->right_cnt, gestureReceived);
        ...
        if (gestureReceived || xTaskGetTickCount() - t > (300 / portTICK_RATE_MS)) {
            if (gestureReceived) {
                ESP_LOGI(APDS_DBG_TAG, ">>> GESTO DECIDIDO: %d (1=UP 2=DOWN 3=LEFT 4=RIGHT) <<<", gestureReceived);
            } else {
                ESP_LOGI(APDS_DBG_TAG, "=== fin (timeout 300ms sin resolver direccion) ===");
            }
            apds9960_reset_counts(sensor);
...
esp_err_t apds9960_gesture_init(apds9960_handle_t sensor)
{
    ...
    /* FIX 3: el PDATA (proximidad cruda) se quedaba siempre en 0 con la
     * mano moviéndose delante del sensor, mientras que la luz ambiental
     * (CDATA/R/G/B) sí reaccionaba. Eso confirma que el I2C y el sensor
     * están bien, pero la señal IR reflejada no llegaba con fuerza
     * suficiente: la ganancia de proximidad nunca se configuraba
     * explícitamente (se quedaba en el valor mínimo de reset, 1x) y el
     * boost del LED estaba en 100% (el mínimo posible, la escala real es
     * 100/150/200/300%). Se sube ganancia a 8x (máxima) y boost a 300%
     * (máximo) para que el motor de gestos realmente vea algo. */
    apds9960_set_proximity_gain(sensor, APDS9960_PGAIN_8X);
    apds9960_set_gesture_proximity_threshold(sensor, 15, 0);
    apds9960_reset_counts(sensor);
    apds9960_set_led_drive_boost(sensor, APDS9960_LEDDRIVE_100MA, APDS9960_LEDBOOST_300PCNT);
